...
void Data_Send_Scope(void)
{
	...
	DataScope_Get_Channel_Data(GYRO_Real.Z, 1);
	DataScope_Get_Channel_Data(Attitude_Angle.Y, 2);
	DataScope_Get_Channel_Data(MOTOR_Speed_Left, 3);
	DataScope_Get_Channel_Data(MOTOR_Speed_Right, 4);
	DataScope_Get_Channel_Data(MOTOR_Left_Acc, 5);
	DataScope_Get_Channel_Data(MOTOR_Right_Acc, 6);
	DataScope_Get_Channel_Data(Tar_Ang_Vel.Z, 7);
	DataScope_Get_Channel_Data(Vel_Set, 8);
	
	Send_Count = DataScope_Data_Generate(10);
	uart_putbuff(UART4, DataScope_OutPut_Buffer, Send_Count);	// 使用串口发送
}
